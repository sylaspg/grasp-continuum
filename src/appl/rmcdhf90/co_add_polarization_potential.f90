...
!***********************************************************************
    ...
    IF (first_call) THEN
        ...
        WRITE(ISTDE,*) "Setting up the polarization potential"
        ...
        IF (CO_POLARIZATION_AUTO) THEN
            ...
            CASE (116) ! Lv
                ...
                WRITE(ISTDE,*) "No dipole polarizability is known for Lv, polarization potential will not be included."
            CASE (117) ! Ts
                CO_ALPHA_D = 76.0D0
            CASE (118) ! Oganesson
                CO_ALPHA_D = 58.0D0
            CASE DEFAULT
                CO_ALPHA_D = 0.0
                CO_INCLUDE_POLARIZATION = .FALSE.
                WRITE(ISTDE,*) "No dipole polarization is found for the element, polarization potential will not be included."
            END SELECT

            ! Take cut-off from bound calculations (as size of the outermost bound orbital)
            CO_R0_3 = RINT(CO_ORBITAL-1,CO_ORBITAL-1, 3)
            WRITE(ISTDE,'(A,I2,A,A)') " <R0^3> is taken from size of the ", &
